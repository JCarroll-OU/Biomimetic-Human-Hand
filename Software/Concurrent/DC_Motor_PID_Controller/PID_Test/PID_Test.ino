...
#define JOINT_DEFAULT_MAX_DEG 180.0
...
// PID Test uses all the code from the parent but does not accept serial commands, instead it sets the targets to 90* 
// and constantly outputs the current position of the potentiometer and the and PID controller. 
void setup() {
  Serial.begin(BAUD_RATE);
  Setup_IO();

  for (int i = 0; i < 4; i++)
    PID_TARGETS[i] = 90.00; // Set target to 90 degrees
}
...
void loop() {
  ...
  Serial.println("PID_1 Target: 90.00 (deg), Current: " + String(PID_INPUTS[0]) + " (deg), PID_Output: " + String(MOTOR_SPEEDS[0]));
